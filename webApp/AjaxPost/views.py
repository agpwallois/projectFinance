...
def handle_post_request(request,id):
	...
	try: 
		...
		financial_model = WindProject(request).to_dict()
		financial_model2 = SolarProject(request).to_dict()


		inp = create_input_dict(request)
		date_series = create_dates_series(inp)
		flags = create_flags(inp, date_series)
		days_series = create_days_series(flags, date_series)
		time_series = create_time_series(date_series, days_series, flags)
		seasonality = create_season_series(request,date_series)
		capacity = calc_capacity(inp, flags, time_series)
		construction_costs,construction_costs_cumul,sum_construction_costs  = calc_construction_costs(request,inp,flags)
		



		dev_tax_commune = import_dev_tax_commune(request, inp['commune'])
		development_tax,archeological_tax,local_taxes = calc_local_taxes(request,inp,flags,dev_tax_commune)
		development_tax_sum = sum(development_tax)
		archeological_tax_sum = sum(archeological_tax)
		sum_local_taxes = sum(local_taxes)

		gross_property_value = sum_construction_costs*property_tax[inp['technology']]["Tax base"]*property_tax[inp['technology']]["Annual property rental value"]*flags['operations']
		gross_land_value = inp['lease']/property_tax[inp['technology']]["Land discount rate"]*property_tax[inp['technology']]["Annual land value"]*flags['operations']
		
		total_taxable_value = (gross_property_value+gross_land_value)*tfpb_standard_reduction
		tfpb = inp['tfpb_rate']*total_taxable_value*flags['start_calendar_year']



		cfe_std_deduction = gross_property_value*inp['cfe_discount_tax_base']
		cfe_tax_base = gross_property_value-cfe_std_deduction
		cfe_due = cfe_tax_base*inp['cfe_rate']*flags['start_calendar_year']
		cfe_mgt_fee = cfe_tax_base*inp['cfe_discount_tax_base']*flags['start_calendar_year']
		cfe_total_due = cfe_due+cfe_mgt_fee



		df_test = pd.DataFrame(data)
		if inp['commune'] not in df_test["Commune"].values:
			tfpb_commune = 0
			tfpb_intercommunal = 0
			tfpb_specific_eqpt = 0		
		else: 
			index_of_abergement = data["Commune"].index(inp['commune'])
		...
		dict_scenario = create_scenario_dict(inp)
		...
		for key in dict_scenario:
			...
			capacity_factor = calc_capacity_factor(production,inp['installed_capacity'],days_series['days_operations'])
			...
			contract_price = calc_contract_price(inp['contract'],inp['wind_turbine_installed'], inp['rotor_diameter'],production_under_contract, production_under_contract_cumul, capacity_factor, days_series,time_series,flags)
		
			index = calc_index(inp, time_series, dict_scenario, key)

			prices, dic_price_elec_keys = calc_prices(request, time_series, inp, flags, index, dict_scenario, key)

			revenues = calc_revenues(production, prices, time_series)
			opex = inp['opex']*index['opex']*time_series['years_during_operations']*(1+dict_scenario[key]["sensi_opex"])
			lease = inp['lease']*index['lease']*time_series['years_during_operations']*(1+dict_scenario[key]["sensi_opex"])
			...
			working_cap = calc_wc(inp, days_series,revenues,opex,lease)
			...
			if key.startswith('Sensi'):
				...
				target_debt_amount = sum_construction_costs*inp['debt_gearing_max']
			...
			for i in range(30):
				...
				injections = calc_injections(inp, total_uses_cumul, total_costs, total_uses, debt_amount)
				senior_debt = calc_senior_debt(injections['debt'], senior_debt_repayments, inp, days_series, flags, debt_amount)
				...
				total_uses_cumul = total_uses.cumsum()
				depreciation = calc_depreciation(sum_construction_costs, capitalised_fees_cumul, optimised_devfee, time_series, inp,local_taxes)
				income_statement = calc_income_statement(depreciation, EBITDA, senior_debt['interests_operations'], SHL_interests_operations, inp)				
				...
				debt_amount_DSCR = calc_debt_amount_DSCR(CFADS_amo, inp, flags, discount_factor)
				...
				development_fee = inp['devfee_paid_FC'] * optimised_devfee * flags['construction_start'] + inp['devfee_paid_COD'] * optimised_devfee * flags['construction_end']
				...
				debt_amount_gearing = total_costs*inp['debt_gearing_max']	
				...
				# Calculate DSRA
				DSRA, DSRA_initial_funding, DSRA_initial_funding_max, dsra_bop = calc_dsra(CFADS, DS_effective, inp, flags, dsra_bop)
				

				cash_available_for_distribution = (CFADS - senior_debt['interests_operations'] - senior_debt_repayments - DSRA['dsra_mov'] - inp['cash_min']*flags['operations'])
				...
				distribution_account = calc_distr_account(SHL_balance_bop, days_series, inp, flags,
			...
			try:
				...
				time_difference = payback_date-inp['construction_start']
			...
			tenor_debt = calculate_tenor(final_repayment_date_debt,inp['construction_start'])


			# Audit and checks 
			audit, check, check_all = calc_audit(total_uses, total_sources, total_assets, total_liabilities, final_repayment_date_debt, inp)
			...
			COD_formatted = format_single_date(inp['COD'])
			end_of_operations_formatted = format_single_date(inp['end_of_operations'])
			liquidation_formatted = format_single_date(inp['liquidation_date'])
			debt_maturity_formatted = format_single_date(inp['debt_maturity'])
			...
			valuation = calc_valuation(inp['valuation_df'],date_series['end_period'],equity_cash_flows)



			builder = DictionaryBuilder()
			dict_to_display = builder.build_merged_dictionary(
				archeological_tax,
				audit,
				capacity,
				capacity_factor,
				...
				local_taxes,
				opex,
				operating_account_bop,
				operating_account_eop,
				PLCR,
				PPE,
				prices,
				production,
				production_under_contract_cumul,
				revenues,
				seasonality,
				senior_debt,
				senior_debt_repayments,
				senior_debt_repayments_target,
				SHL_interests_construction,
				SHL_interests_operations,
				share_capital_bop,
				share_capital_eop,
				share_capital_repayment,
				tfpb,
				time_series,
				total_assets,
				total_cash,
				...
				total_uses,
				total_uses_cumul,
				transfers_distribution_account,
				working_cap,
				)
		...
		key = select_scenario_to_display(project_form)
		...
		if not inp['financial_close_check']:
		...
		return JsonResponse({
			"calculation_detail":inp['calculation_detail'],
			...
			"financial_model2":financial_model2,
...
def create_input_dict(request):
	...
	inp = {
		...
		'opex_index_start_date': pd.Timestamp(post_data['opex_indexation_start_date']),
		'merchant_index_start_date': pd.Timestamp(post_data['price_elec_indexation_start_date']),
		'contract_index_start_date': pd.Timestamp(post_data['contract_indexation_start_date']),
		...
		'lease_index_start_date': pd.Timestamp(post_data['lease_indexation_start_date']),
		...
		'installed_capacity': int(post_data['wind_turbine_installed'])*1000*float(post_data['capacity_per_turbine']),
...
def create_scenario_dict(inp):

		dict_scenario = {
			"Lender Case": {
				"prod": inp['lender_prod'],
				"elec_price": inp['lender_mkt_prices'],
				"sensi_prod": 0,
				"sensi_inf": 0,
				"sensi_opex": 0,
			},
			...
			"Sensi Production": {
				"prod": inp['lender_prod'],
				"elec_price": inp['lender_mkt_prices'],
				"sensi_prod": inp['sensi_production'],
				"sensi_inf": 0,
				"sensi_opex": 0,
			},
			"Sensi Inflation": {
			...
			"Sensi Opex": {
				...
				"sensi_prod": 0,
				"sensi_inf": 0,
				"sensi_opex": inp['sensi_opex'],
			},
			"Sponsor Case": {
				"prod": inp['sponsor_prod'],
				"elec_price": inp['sponsor_mkt_prices'],
				"sensi_prod": 0,
				"sensi_inf": 0,
				"sensi_opex": 0,
			},
...
def select_scenario_to_display(project_form):
...
def create_days_series(flags, date_series):

	days_period = ((date_series['end_period'] - date_series['start_period']).dt.days + 1) * 1
	days_contracted = Days(flags['contract']).create_number_days_series(date_series['end_contract'],date_series['start_contract']) 
	days_contracted_index = Days(flags['contract_index_period']).create_number_days_series(date_series['end_contract_index'],date_series['start_contract_index']) 
	days_merchant_index = Days(flags['merchant_index_period']).create_number_days_series(date_series['end_elec_index'],date_series['start_elec_index']) 
	days_opex_index = Days(flags['opex_index_period']).create_number_days_series(date_series['end_opex_index'],date_series['start_opex_index']) 
	days_lease_index = Days(flags['lease_index_period']).create_number_days_series(date_series['end_lease_index'],date_series['start_lease_index']) 
	days_debt_interest_construction = Days(flags['construction']).create_number_days_series(date_series['end_debt_interests_construction'],date_series['start_debt_interests_construction']) 
	days_debt_interest_operations = Days(flags['debt_amo']).create_number_days_series(date_series['end_debt_interests_operations'],date_series['start_debt_interests_operations']) 
	days_days_operations = Days(flags['operations']).create_number_days_series(date_series['end_operations'],date_series['start_operations']) 

	days_dict = {
	'period': days_period,
	'contracted': days_contracted,
	'contracted_index': days_contracted_index,
	'merchant_index': days_merchant_index,
	'opex_index': days_opex_index,
	'debt_interest_construction': days_debt_interest_construction,
	'debt_interest_operations': days_debt_interest_operations,
	'days_operations': days_days_operations,
	'lease_index': days_lease_index,
	}
...
class DictionaryBuilder:
	...
	@staticmethod
	def get_flags_m(time_series, days_series, flags):
		return {
	...
	@staticmethod
	def get_flag_oftk_i(flags, date_series, days_series, index):
		return {
	...
	@staticmethod
	def get_flag_op(flags, time_series, days_series, seasonality):
		return {
	...
	@staticmethod
	def get_flag_fin(flags, days_series):
		return {
	...
	@staticmethod
	def get_is(revenues, opex, lease, tfpb, EBITDA, income_statement, depreciation, senior_debt, SHL_interests_operations):
		return {
	...
	@staticmethod
	def get_mkt_i(index, days_series, date_series, flags):
		return {
	...
	@staticmethod
	def get_opex(index, days_series, date_series, flags, time_series):
		return {
			...
			"Years from indexation start date": time_series["years_from_base_date_opex_index"].tolist()
		}

	@staticmethod
	def get_lease(index, days_series, date_series, flags, time_series):
		return {
			...
			"Years from indexation start date": time_series["years_from_base_date_lease_index"].tolist()
		}

	@staticmethod
	def get_price(prices, contract_price):
		return {
	...
	@staticmethod
	def get_prod(capacity, production, capacity_factor, production_under_contract_cumul):
		return {
	...
	@staticmethod
	def get_wc_rec(working_cap, revenues):
		return {
	...
	@staticmethod
	def get_wc_pay(working_cap, opex, lease):
		return {
	...
	@staticmethod
	def get_cf_op(EBITDA, working_cap, income_statement, cf_statement):
		return {
			"EBITDA": EBITDA.tolist(),
	...
	@staticmethod
	def get_cf_in(construction_costs, development_tax, archeological_tax, development_fee, senior_debt, cf_statement):
		return {
	...
	@staticmethod
	def get_cf_fi(senior_debt, injections, cf_statement):
		return {
	...
	@staticmethod
	def get_cfads(CFADS, senior_debt, senior_debt_repayments):
		return {
			"CFADS": CFADS.tolist(),
			"Senior debt interests": (-1 * senior_debt['interests_operations']).tolist(),
	...
	@staticmethod
	def get_cf_distr(cash_available_for_distribution, transfers_distribution_account):
		return {
	...
	@staticmethod
	def get_op_acc(operating_account_bop, operating_account_eop):
		return {
	...
	@staticmethod
	def get_fp_u(construction_costs, development_fee, development_tax, archeological_tax, senior_debt, DSRA_initial_funding, total_uses):
		return {
	...
	@staticmethod
	def get_fp_s(injections, total_sources):
		return {
			"Senior debt drawdowns": injections['debt'].tolist(),
	...
	@staticmethod
	def get_debt_a(senior_debt, injections):
		return {
	...
	@staticmethod
	def get_debt_b(senior_debt, injections, senior_debt_repayments):
		return {
	...
	@staticmethod
	def get_sizing(CFADS_amo, debt_amount_DSCR, discount_factor, senior_debt, senior_debt_repayments_target):
		return {
	...
	@staticmethod
	def get_distrib_bop(distribution_account, transfers_distribution_account):
		return {
	...
	@staticmethod
	def get_distrib_shli(distribution_account):
		return {
	...
	@staticmethod
	def get_distrib_div(distribution_account):
		return {
	...
	@staticmethod
	def get_distrib_shlp(distribution_account):
		return {
	...
	@staticmethod
	def get_distrib_sc(distribution_account, share_capital_repayment):
		return {
	...
	@staticmethod
	def get_distrib_eop(distribution_account_eop):
		return {
			"Distribution account balance": distribution_account_eop.tolist()
		}

	@staticmethod
	def get_dev_tax(development_tax):
		return {
			"Development tax": development_tax.tolist()
		}

	@staticmethod
	def get_arch_tax(archeological_tax):
		return {
			"Archaeological tax": archeological_tax.tolist()
		}

	@staticmethod
	def get_prop_tax(gross_property_value, gross_land_value, tfpb):
		return {
	...
	@staticmethod
	def get_cfe_tax_base(gross_property_value, cfe_std_deduction, cfe_tax_base, cfe_due, cfe_mgt_fee, cfe_total_due):
		return {
	...
	@staticmethod
	def get_shl(distribution_account, injections, SHL_interests_construction):
		return {
	...
	@staticmethod
	def get_ishl(SHL_interests_construction, SHL_interests_operations):
		return {
	...
	@staticmethod
	def get_re_b(distribution_account, income_statement):
		return {
	...
	@staticmethod
	def get_eqt(share_capital_bop, injections, share_capital_repayment, share_capital_eop):
		return {
	...
	@staticmethod
	def get_bs_a(PPE, working_cap, total_cash, operating_account_eop, DSRA, distribution_account_eop, total_assets):
		return {
	...
	@staticmethod
	def get_bs_l(share_capital_eop, distribution_account, senior_debt, working_cap, total_liabilities):
		return {
	...
	@staticmethod
	def get_ratio(DSCR_effective, LLCR, PLCR):
		return {
	...
	@staticmethod
	def get_graphs(total_uses_cumul, injections, distribution_account, lease, opex, tfpb, senior_debt_repayments, EBITDA_margin, construction_costs_cumul, share_capital_repayment, DS_effective, irr_values, gearing_during_finplan, debt_fees, local_taxes):
		return {
	...
	def build_merged_dictionary(self, 
	archeological_tax, 
	audit, 
	capacity, 
	capacity_factor, 
	...
	working_cap, 
		...
		return {
			"Dates": self.get_dates(date_series),
			"FlagsC": self.get_flags_c(flags),
			"FlagsM": self.get_flags_m(time_series, days_series, flags),
			"FlagOftk_i": self.get_flag_oftk_i(flags, date_series, days_series, index),
			"FlagOp": self.get_flag_op(flags, time_series, days_series, seasonality),
			"FlagFin": self.get_flag_fin(flags, days_series),
			"IS": self.get_is(revenues, opex, lease, tfpb, EBITDA, income_statement, depreciation, senior_debt, SHL_interests_operations),
			"Mkt_i": self.get_mkt_i(index, days_series, date_series, flags),
			"Opex": self.get_opex(index, days_series, date_series, flags, time_series),
			"Lease": self.get_lease(index, days_series, date_series, flags, time_series),
			"Price": self.get_price(prices, contract_price),
			"Prod": self.get_prod(capacity, production, capacity_factor, production_under_contract_cumul),
			"WCRec": self.get_wc_rec(working_cap, revenues),
			"WCPay": self.get_wc_pay(working_cap, opex, lease),
			"CF_op": self.get_cf_op(EBITDA, working_cap, income_statement, cf_statement),
			"CF_in": self.get_cf_in(construction_costs, development_tax, archeological_tax, development_fee, senior_debt, cf_statement),
			"CF_fi": self.get_cf_fi(senior_debt, injections, cf_statement),
			"WCMov": self.get_wc_mov(working_cap),
			"CFADS": self.get_cfads(CFADS, senior_debt, senior_debt_repayments),
			"CFDSRA": self.get_cf_dsra(DSRA),
			"CFDistr": self.get_cf_distr(cash_available_for_distribution, transfers_distribution_account),
			"OpAcc": self.get_op_acc(operating_account_bop, operating_account_eop),
			"FP_u": self.get_fp_u(construction_costs, development_fee, development_tax, archeological_tax, senior_debt, DSRA_initial_funding, total_uses),
			"FP_s": self.get_fp_s(injections, total_sources),
			"Debt_a": self.get_debt_a(senior_debt, injections),
			"Debt_b": self.get_debt_b(senior_debt, injections, senior_debt_repayments),
			"Debt_i": self.get_debt_i(senior_debt),
			"Sizing": self.get_sizing(CFADS_amo, debt_amount_DSCR, discount_factor, senior_debt, senior_debt_repayments_target),
			"DSRA": self.get_dsra(DSRA),
			"Distrib_BOP": self.get_distrib_bop(distribution_account, transfers_distribution_account),
			"Distrib_SHLi": self.get_distrib_shli(distribution_account),
			"Distrib_Div": self.get_distrib_div(distribution_account),
			"Distrib_SHLp": self.get_distrib_shlp(distribution_account),
			"Distrib_SC": self.get_distrib_sc(distribution_account, share_capital_repayment),
			"Distrib_EOP": self.get_distrib_eop(distribution_account_eop),
			"DevTax": self.get_dev_tax(development_tax),
			"ArchTax": self.get_arch_tax(archeological_tax),
			"PropTax": self.get_prop_tax(gross_property_value, gross_land_value, tfpb),
			"CFETaxBase": self.get_cfe_tax_base(gross_property_value, cfe_std_deduction, cfe_tax_base, cfe_due, cfe_mgt_fee, cfe_total_due),
			"SHL": self.get_shl(distribution_account, injections, SHL_interests_construction),
			"iSHL": self.get_ishl(SHL_interests_construction, SHL_interests_operations),
			"RE_b": self.get_re_b(distribution_account, income_statement),
			"Eqt": self.get_eqt(share_capital_bop, injections, share_capital_repayment, share_capital_eop),
			"BS_a": self.get_bs_a(PPE, working_cap, total_cash, operating_account_eop, DSRA, distribution_account_eop, total_assets),
			"BS_l": self.get_bs_l(share_capital_eop, distribution_account, senior_debt, working_cap, total_liabilities),
			"Ratio": self.get_ratio(DSCR_effective, LLCR, PLCR),
			"Audit": self.get_audit(audit),
			"Graphs": self.get_graphs(total_uses_cumul, injections, distribution_account, lease, opex, tfpb, senior_debt_repayments, EBITDA_margin, construction_costs_cumul, share_capital_repayment, DS_effective, irr_values, gearing_during_finplan, debt_fees, local_taxes)
			}
...
def calc_construction_costs(request, inp, flags):
	
	inp_construction_costs = get_constr_costs(request, inp)
...
def calc_index(inp, time_series, dict_scenario, key):

	sentivity_inflation = dict_scenario[key]["sensi_inf"]
	
	elec_index_indice = array_index(inp['index_rate_merchant']+sentivity_inflation,time_series['years_from_base_date_merchant_index'])
	contract_index_indice = array_index(inp['index_rate_contract']+sentivity_inflation,time_series['years_from_base_date_contracted_index'])
	opex_index_indice = array_index(inp['index_rate_opex']+sentivity_inflation,time_series['years_from_base_date_opex_index'])
	lease_index_indice = array_index(inp['index_rate_lease']+sentivity_inflation,time_series['years_from_base_date_lease_index'])
...
def calc_prices(request, time_series, inp, flags, index, dict_scenario, key):


	dic_price_elec, dic_price_elec_keys = get_elec_prices(request, inp['construction_end'],inp['liquidation_date'], dict_scenario, key)



	merchant_prices_real = array_elec_prices(time_series['series_end_period_year'], dic_price_elec)
	contracted_prices_real = inp['contract_price'] * flags['contract']
...
def calc_wc(inp, days_series, revenues, opex,lease):
	revenues_in_period_paid = (1 - inp['payment_delay_revenues'] / days_series['period']) * revenues['total']
	...
	costs_in_period_paid = (1 - inp['payment_delay_costs'] / days_series['period']) * (opex+lease)
...
def calc_distr_account(SHL_balance_bop, days_series, inp, flags,
						  transfers_distribution_account, distributable_profit, injections, income_statement):
	...
	for i in range(100):
		...
		SHL_interests_operations = SHL_balance_bop * inp['SHL_margin'] * period / 360 * flag_operations
		SHL_interests_construction = SHL_balance_bop * inp['SHL_margin'] * period / 360 * flag_construction
...
def calc_injections(inp, total_uses_cumul, total_costs, total_uses, debt_amount):
	

	equity_amount= total_costs-debt_amount
	gearing_eff = (debt_amount/total_costs)

	if inp['injection_choice'] == 1:
		...
		share_capital_injections = equity_injections*(1-inp['subgearing'])
		SHL_injections = equity_injections*inp['subgearing']
		senior_debt_drawdowns = total_uses - equity_injections
		senior_debt_drawdowns_cumul = senior_debt_drawdowns.cumsum()
	elif inp['injection_choice'] == 2:
...
def calc_senior_debt(senior_debt_drawdowns, senior_debt_repayments, inp, days_series, flags, debt_amount):
	...
	interests_construction = balance_bop * inp['debt_interest_rate'] * days_series['debt_interest_construction'] / 360
	interests_operations = balance_bop * inp['debt_interest_rate'] * days_series['debt_interest_operations'] / 360

	interests = interests_construction + interests_operations

	upfront_fee=flags['construction_start']*debt_amount*inp['debt_upfront_fee']
	...
	commitment_fees=senior_debt_available_bop*inp['debt_commitment_fee']*days_series['period']/360
...
def calc_depreciation(sum_construction_costs, capitalised_fees_cumul, optimised_devfee, time_series, inp,local_taxes):

	total_uses_depreciable = sum_construction_costs+max(capitalised_fees_cumul)+optimised_devfee+local_taxes
	depreciation = 	total_uses_depreciable*time_series['years_during_operations']/inp['operating_life']

	return depreciation	



def calc_income_statement(depreciation, EBITDA, senior_debt_interests_operations, SHL_interests_operations, inp):
	EBIT = EBITDA - depreciation
	EBT = EBIT - senior_debt_interests_operations - SHL_interests_operations
	corporate_income_tax = np.clip(inp['income_tax_rate'] * EBT, 0, None)
...
def calc_debt_amount_DSCR(CFADS_amo, inp, flags,discount_factor):

	target_DSCR = inp['debt_target_DSCR']*flags['debt_amo']
	target_DS = CFADS_amo/inp['debt_target_DSCR']
...
def calc_dsra(CFADS, DS_effective, inp, flags, dsra_bop):
	cash_available_for_dsra = np.maximum(CFADS - DS_effective, 0)
	dsra_target = calc_dsra_target(inp, DS_effective) * flags['debt_amo']
...
def calc_audit(total_uses, total_sources, total_assets, total_liabilities, final_repayment_date_debt, inp):
	...
	check_debt_maturity = (final_repayment_date_debt == inp['debt_maturity'])		
...
def calc_dsra_target(inp,DS_effective):

	look_forward=int(inp['dsra']/inp['periodicity'])	
...
def get_constr_costs(request, inp):
	construction_costs = np.zeros(24)

	delta = relativedelta(inp['construction_end'], inp['construction_start'])
...
def create_period_series(series_start_period, series_end_period, inp):
	dates = {
		'contract': (inp['start_contract'], inp['end_contract']),
		'contract_index': (inp['contract_index_start_date'], inp['end_contract']),
		'elec_index': (inp['merchant_index_start_date'], inp['end_of_operations']),
		'opex_index': (inp['opex_index_start_date'], inp['end_of_operations']),
		'debt_interests_construction': (inp['construction_start'], inp['construction_end']),
		'debt_interests_operations': (inp['COD'], inp['debt_maturity_date']),
		'lease_index': (inp['lease_index_start_date'], inp['end_of_operations']),

		'operations': (inp['COD'], inp['end_of_operations']),
...
def calc_local_taxes(request,inp,flags,dev_tax_commune):

	total_rate=dev_tax_commune/100+inp['dev_tax_department_tax']
	
	if inp['technology'].startswith('Solar'):
		development_tax=inp['panels_surface']*inp['dev_tax_taxable_base_solar']*total_rate/1000*flags['construction_start']
		archeological_tax=inp['panels_surface']*inp['archeological_tax_base_solar']*inp['archeological_tax']/1000*flags['construction_start']

	else: 
		development_tax=inp['wind_turbine_installed']*inp['dev_tax_taxable_base_wind']*total_rate/1000*flags['construction_start']
...
def first_day_next_month(date,inp):
	first_day_next_month = date.replace(day=1) + relativedelta(months=inp['periodicity']) + datetime.timedelta(days=-1)
...
def calc_capacity(inp, flags, time_series):

	if inp['technology'].startswith('Solar'):

		capacity_before_degradation = inp['panels_capacity']*flags['operations']
		degradation_factor = 1/(1+inp['degradation'])**time_series['years_from_COD_avg']
		capacity_after_degradation = capacity_before_degradation * degradation_factor

	else: 

		capacity_before_degradation = inp['wind_turbine_installed']*1000*inp['capacity_per_turbine']*flags['operations']
...
def create_season_series(request,date_series):
	
	inp_seasonality = import_seasonality(request)

	data = {'start':date_series['start_period'],
			'end':date_series['end_period']}

	df = pd.DataFrame(data)

	df_seasonality_result = pd.DataFrame(columns=date_series['end_period'])
...
def create_dates_series(inp):
	series_start_period, series_end_period = create_timeline_series(inp)
	series = create_period_series(series_start_period, series_end_period, inp)
...
def create_timeline_series(inp):

	freq_period_start = str(inp['periodicity'])+"MS"
	freq_period_end = str(inp['periodicity'])+"M"

	test = int(inp['periodicity']/3)

	first_day_month_construction_start = first_day_month(inp['construction_start'])
	last_day_construction_end = last_day_month(inp['construction_end'])
	...
	last_day_liquidation_end = last_day_month(inp['liquidation_date'])

	last_day_end_liquidation_plus_freq = first_day_next_month(inp['liquidation_date'],inp)
...
def create_time_series(date_series, days_series, flags):
	...
	time_series_results = {
		...
		'quarters': quarters,
		'years_from_base_date_contracted_index': years_from_base_dates['contracted_index'],
		'years_from_base_date_merchant_index': years_from_base_dates['merchant_index'],
		'years_from_base_date_opex_index': years_from_base_dates['opex_index'],
		'years_from_base_date_lease_index': years_from_base_dates['lease_index'],
...
def create_flags(inp, date_series):

	flag = Flag(date_series['start_period'], date_series['end_period'])

	flag_construction_start = flag.create_flag(inp['construction_start'],inp['construction_start'])
	flag_construction_end = flag.create_flag(inp['construction_end'],inp['construction_end'])
	flag_construction = flag.create_flag(inp['construction_start'],inp['construction_end'])
	flag_operations = flag.create_flag(inp['COD'],inp['end_of_operations'])
	flag_contract = flag.create_flag(inp['start_contract'],inp['end_contract'])

	flag_start_year = flag.create_flag_start_year()
	

	flag_operations_end = flag.create_flag(inp['end_of_operations'],inp['end_of_operations'])
	flag_liquidation = flag.create_flag(inp['end_of_operations']+datetime.timedelta(days=1),inp['liquidation_date'])		
	flag_liquidation_end = flag.create_flag(inp['liquidation_date'],inp['liquidation_date'])
	flag_debt_amo = flag.create_flag(inp['COD'],inp['debt_maturity'])
	flag_contract_index_period = flag.create_flag(date_series['start_contract_index'],inp['end_contract'])
	flag_merchant_index_period = flag.create_flag(date_series['start_elec_index'],inp['end_of_operations'])
	flag_opex_index_period = flag.create_flag(date_series['start_opex_index'],inp['end_of_operations'])
	flag_lease_index_period = flag.create_flag(date_series['start_lease_index'],inp['end_of_operations'])

	flags_dict = {
	'operations': flag_operations,
	'construction': flag_construction,
	'construction_end': flag_construction_end,
	'construction_start': flag_construction_start,
	'liquidation': flag_liquidation,
	'liquidation_end': flag_liquidation_end,
	'debt_amo': flag_debt_amo,
	'contract': flag_contract,
	'contract_index_period': flag_contract_index_period,
	'merchant_index_period': flag_merchant_index_period,
	'opex_index_period': flag_opex_index_period,
	'lease_index_period': flag_lease_index_period,
	'start_calendar_year': flag_start_year,
	}
